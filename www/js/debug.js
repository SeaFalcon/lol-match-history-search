...
.controller('ApiCtrl', function($scope, $http){
	...
	$scope.showDetail = function(matchSummary, idx){
		...
		var endPoint = 'api/lol/kr/v2.2/match/'+ matchSummary.gameId +'?api_key=' + apiKey;
		$http.get(endPoint).success(function(response){
			...
			$http.get('api/lol/kr/v1.4/summoner/'+fellowPlayers.join()+'/name?api_key='+apiKey)
			...
			$http.get('/api/lol/kr/v2.5/league/by-summoner/'+ fellowPlayers.join() +'/entry?api_key=' + apiKey).
			success(function(response){
				for(i=0; i<matchSummary.fellowPlayers.length; i++){
	...
	$http.get('js/static_data.json').success(function(response){
	...
	$scope.loadUserId = function(){
		...
		var summonerName = $('#summonerName').val().toLowerCase();
		var sName = summonerName.split(' ').join('');
		var endPoint = '/api/lol/kr/v1.4/summoner/by-name/'+ summonerName +'?api_key=' + apiKey;
		$http.get(endPoint).success(function(response){
			...
			if(response){
				...
				$scope.status = '200';
				$http.get('api/lol/kr/v1.3/stats/by-summoner/'+ response[sName].id +'/summary?api_key='+apiKey).success(function(response){
	...
	function leagueEntry(summonerId){
		var endPoint = '/api/lol/kr/v2.5/league/by-summoner/'+ summonerId +'/entry?api_key=' + apiKey;
		$http.get(endPoint).success(function(response){
	...
	function loadRecentGameList(summonerId){
		...
		var endPoint = '/api/lol/kr/v1.3/game/by-summoner/' + summonerId + '/recent?api_key=' + apiKey;
		$http.get(endPoint).success(function(response){
	...
	function loadMatchInfo(matchId){
		var endPoint = '/api/lol/kr/v2.2/match/' + matchId + '?api_key=' + apiKey;
		$http.get(endPoint).success(function(response){
