angular.module('riot.controllers', [])
  ...
  .controller('ApiCtrl', function($scope, $http){
    var prefix = 'https://kr.api.pvp.net/api/lol/kr';
    ...
    $scope.showDetail = function(matchSummary, idx){
      ...
      var endPoint = prefix+'/v2.2/match/'+ matchSummary.gameId +'?api_key=' + apiKey;
      $http.get(endPoint).success(function(response){
        ...
        $http.get(prefix+'/v1.4/summoner/'+fellowPlayers.join()+'/name?api_key='+apiKey)
        ...
        $http.get(prefix+'/v2.5/league/by-summoner/'+ fellowPlayers.join() +'/entry?api_key=' + apiKey).
        success(function(response){
          for(i=0; i<matchSummary.fellowPlayers.length; i++){
    ...
    $scope.loadUserId = function(sn){
      ...
      var summonerName = sn.toLowerCase();
      var sName = summonerName.split(' ').join('');
      var endPoint = prefix+'/v1.4/summoner/by-name/'+ summonerName +'?api_key=' + apiKey;
      $http.get(endPoint).success(function(response){
        ...
        if(response){
          ...
          $scope.status = '200';
          $http.get(prefix+'/v1.3/stats/by-summoner/'+ response[sName].id +'/summary?api_key='+apiKey).success(function(response){
    ...
    function leagueEntry(summonerId){
      var endPoint = prefix+'/v2.5/league/by-summoner/'+ summonerId +'/entry?api_key=' + apiKey;
      $http.get(endPoint).success(function(response){
    ...
    function loadRecentGameList(summonerId){
      ...
      var endPoint = prefix+'/v1.3/game/by-summoner/' + summonerId + '/recent?api_key=' + apiKey;
      $http.get(endPoint).success(function(response){
